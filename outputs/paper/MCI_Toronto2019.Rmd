...
mci_selected <- mci %>% select("MCI", "occurrencedate", "reporteddate", "offence", "Neighbourhood", "Lat", "Long" )

mci_selected <- mci_selected[1:10, 1:7]
mci_selected$Lat <- format(round(mci_selected$Lat, 2), nsmall = 2)
mci_selected$Long <- format(round(mci_selected$Long, 2), nsmall = 2)

mci_selected <- mci_selected %>% rename( 
  Occurrence = occurrencedate, Reported = reporteddate, Offence = offence, Hood = Neighbourhood)
