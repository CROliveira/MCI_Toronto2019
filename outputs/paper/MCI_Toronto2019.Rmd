...
This study does not intend to exhaust correlation nor find causation to support any evidence related to major crime indicator occurrences, order, motivation, aggravated factors nor to recommend mitigation practices. However, it is presented a list of 9 (nine) variables that I aim to explore as follows:
...
mci$occurrencedate <- as.Date( mci$occurrencedate)
mci$reporteddate <- as.Date( mci$reporteddate)


# mci_type <- table(mci$MCI, mci$occurrencedate) # Distinct value count for MCI column
mci_selected <- mci %>% select("MCI", "occurrencedate", "reporteddate","occurrencemonth", "occurrencehour", "offence", "Neighbourhood", "Lat", "Long" )

mci_selected <- mci_selected[1:10, 1:9]

mci_selected <- mci_selected %>% rename( 
  Occurrence = occurrencedate, Reported = reporteddate, Month = occurrencemonth,
  Hour = occurrencehour, Offence = offence, Hood = Neighbourhood)

kbl(mci_selected) %>% kable_styling( 
  bootstrap_options = "striped", font_size = 9, full_width = F, position = "left") %>% 
  row_spec( 0, angle = 0)
